...
bool OpenSMLoading::init(const QString &path){
    ...
    while(!in.atEnd()){
        line = in.readLine();
        if(line.indexOf(lineSearchNodeKey) >= 0){ //寻找结点
            ...
            double y = list[8].mid(5, list[8].size()-6).toDouble();
            double x = list[9].mid(5, list[8].size()-8).toDouble();
            ...
            while(wayline.indexOf("/way") < 0){ //如果找到way 结束这条way的数据载入
...
void OpenSMLoading::initGL(){
    ...
    while(iter != this->map_Ways.end()){ //遍历存储的所有way
        QVector<QVector3D> new_positions;             // 放入buffer进行绘图的点，已经由垂线生成了新的点
        QVector<QVector3D> ori_positions;             // 一条way的初始节点位置
        QVector<QVector2D> uvs;
        QVector<QVector3D> normals;

        if(iter.value().kvPairs.contains("highway")){   // 当前只处理公路的三维化
            for(int i = 1; i != iter.value().nodesID.size(); ++i){ //拿到way的最小与最大xy值
            ...
            for(int i = 1; i != iter.value().nodesID.size(); ++i){ //存后八个矢量差值
            ...
            for(int i = 0; i < ori_positions.size()-1; ++i){         //通过垂线 计算新增添的点
                ...
                new_positions.push_back(next_add_t);
                new_positions.push_back(next_add_b);
                new_positions.push_back(next_b);                //后

                new_positions.push_back(cur_b);                //左
                new_positions.push_back(cur_t);
                new_positions.push_back(next_t);
                new_positions.push_back(next_b);

                new_positions.push_back(cur_add_b);                //右
                new_positions.push_back(next_add_b);
                new_positions.push_back(next_add_t);
                new_positions.push_back(cur_add_t);
            }
            ...
            for(int i = 0; i < new_positions.size(); i += 4){ //通过叉乘计算 立方体一个面的法线， 即一次 刷新四个点
        ...
        if(!new_positions.isEmpty()){
            ...
            core->glBufferData(GL_ARRAY_BUFFER, new_positions.size()* sizeof(QVector3D), &new_positions[0], GL_STATIC_DRAW);
...
bool isPolygonCounterClocked(QVector<QVector2D> &contour){ //判断传进来的多边形数组是否是 逆时针
    if(contour.size() < 3){
